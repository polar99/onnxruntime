...
class TrainingSessionTest : public TrainingSession {
 public:
  TrainingSessionTest(const SessionOptions& session_options,
                                        const Environment& env)
      : TrainingSession(session_options, env) {
  }

  TrainingSessionTest(TrainingSession& ts, const Environment& env):TrainingSession(ts.GetSessionOptions(), env){
    TrainingSession::Initialize();
  }

  const SessionState& GetSessionState() { return TrainingSession::GetSessionState(); }
};
...
void test_tensor_slicing(TrainingSession& session_object){
   ...
   // Prepare inputs and outputs.
    ...
    std::vector<float> value = {0.0f,1.0f,3.0f,2.0f,4.0f,9.0f,3.0f,2.0f,3.0f,
                                4.0f,1.0f,2.0f,6.0f,3.0f,4.0f,1.0f,3.0f,2.0f,
                                0.0f,-1.0f,1.0f,6.0f,4.0f,3.0f,2.0f,9.0f,5.0f,
                                3.0f,4.0f,3.0f,1.0f,2.0f,5.0f,4.0f,3.0f,2.0f};
    // expected res = {3.0f,2.0f,3.0f,   4.0f,1.0f,2.0f,    2.0f,9.0f,5.0f,  3.0f,4.0f,3.0f }
    TrainingUtil::CreateCpuMLValue(shape, value, &input);
    //OrtValue SliceTensor(const OrtValue& orig_value, const size_t slice_id,
    //                  const size_t slice_axis, const size_t num_slices, SessionState& session_state)
    //const SessionState& session_state = session_object.GetSessionState();
    size_t slice_id = 1; // starting from index 0
    size_t slice_axis = 1; // starting from index 0 for axes
    size_t num_slices = 3;
    OrtValue res = SliceTensor(input, slice_id, slice_axis, num_slices, session_object);
    // res shape = 2,2,3
    const Tensor& resTensor = res.Get<Tensor>();
    const float* resTensor_ptr = resTensor.template Data<float>();
    std::cout<<"shape: " << resTensor.Shape() <<std::endl;
