...
def save(save_obj: dict, path):
    """Saves the input dictionary to a binary file or python file-like object specified by path.
    ...
    Args:
        save_obj: dictionary that needs to be saved.
            save_obj should consist of types understandable by hdf5 file format.
...
def load(path, key=None):
    ...
    Loaded dictionary contains numpy equivalents of python data types. For example:
        tensor -> saved as a numpy array and loaded as a numpy array.
